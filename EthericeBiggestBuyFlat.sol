...
contract EthericeBiggestBuy is Ownable, ReentrancyGuard {
    ...
    /* @dev Some address should be excluded from lottery such as the buyback address  */
    ...
    */
    function doStartComp(address _tokenAddress) external onlyOwner {
    ...
    /**
        ...
        @param _winner % sent to the winnner
    */
    ...
    /**
        ...
        over time there's still a chance for other to win the max buy
        @param _val 100 = 10%
     */
    function updateDailyTopBuyReductionPercentage(uint256 _val) external onlyOwner {
        require(_val <= 990, "Percent cant be more than 99%");
        dailyTopBuyReductionPercentage = _val;
    }
    ...
    */
    function excludeAddressFromLottery(address _address, bool _val) external onlyOwner {
        require(! startLottery, "Addresses can't be excluded once the lottery has started");
        excludeFromComp[_address] = _val;
    }
    ...
    */
    function checkForWinner() external nonReentrant {        
        ...
        if (topBuyTodayAmount > topBuy) {
            ...
            topBuy -= topBuy * dailyTopBuyReductionPercentage / 1000;
        }
    ...
    function processWin() internal {
        // Set the new top amount amount for the next lottery
...
//console.log( _winnerAmount);
// console.log( _devAmount);
// console.log( _rewardsAmount);
// 69000000000000000
// 23000000000000000
        if(topBuyTodayAddress != address(0)){
            payable(topBuyTodayAddress).transfer(_winnerAmount);
        }
        
        payable(_tokenContract.dev_addr()).transfer(_devAmount);
        payable(_tokenContract.rewards_addr()).transfer(_rewardsAmount);

        // triggers func on the token contract for lottery win so the lottery win event can come from the token contract
